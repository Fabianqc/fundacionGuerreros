...
export class DoctorService {
  ...
  async update(updateDoctorDto: UpdateDoctorDto) {
    ...
    if (ci_doctor === undefined || tipo_cedula === undefined) {
      throw new BadRequestException(
        'ci_doctor y tipo_cedula son requeridos para actualizar',
      );
    }
    ...
    try {
      const persona = await queryRunner.manager.findOne(Persona, {
        where: { cedula: ci_doctor, tipo_cedula: tipo_cedula },
      });
      if (!persona) {
      ...
      if (especialidades !== undefined) {
        ...
        await queryRunner.manager.delete(DoctorHasEspecialidade, {
          doctor: { id: doctor.id },
        });
      ...
      if (updateDoctorDto.horarios !== undefined) {
        ...
        await this.doctorHorarioService.create(
          updateDoctorDto.horarios,
          doctor,
          queryRunner.manager,
        );
      }
    ...
    return await this.findOne(ci_doctor, tipo_cedula);
  }
  ...
  async findAllWithLimit(skip: number, take: number, search: string) {
    ...
    queryBuilder.leftJoinAndSelect(
      'doctor.doctor_especialidades',
      'doctor_especialidades',
    );
    queryBuilder.leftJoinAndSelect(
      'doctor_especialidades.especialidad',
      'especialidad',
    );
    if (search) {
      queryBuilder.where(
        'persona.fullname ILIKE :search OR persona.cedula ILIKE :search',
        { search: `%${search}%` },
      );
    }
  ...
  async findAll() {
    const doctors = await this.doctorRepository.find({
      relations: ['persona', 'doctor_especialidades.especialidad'],
    });
    return doctors.map((doctor) => {
      const transformed = instanceToPlain(doctor);
      transformed.doctor_especialidades =
        doctor.doctor_especialidades?.map((de) => ({
  ...
  async findOne(ci_doctor: string, tipo_cedula: string) {
    ...
    const transformed = instanceToPlain(doctor);
    transformed.doctor_especialidades =
      doctor.doctor_especialidades?.map((de) => de.especialidad.nombre) || [];
    transformed.horarios = this.doctorHorarioService.reconstructDto(
      doctor.doctor_horario,
    );
    return transformed;
  }
  ...
  async create(createDoctorDto: CreateDoctorDto) {
    ...
    try {
      const persona = await queryRunner.manager.findOne(Persona, {
        where: {
          cedula: createDoctorDto.ci_doctor,
      ...
      const doctorExistente = await queryRunner.manager.findOne(Doctor, {
    ...
    return await this.findOne(
      createDoctorDto.ci_doctor,
      createDoctorDto.tipo_cedula,
    );
  }
}
