...
describe('EspecialidadesService', () => {
  ...
  describe('create', () => {
    it('debería arrojar "BadRequestException" si la especialidad ya existe', async () => {
      mockEspecialidadRepository.findOne.mockResolvedValue({
        id: '1',
        nombre: 'Pediatria',
      });
    ...
    it('debería crear y guardar una especialidad si no existe', async () => {
      ...
      expect(mockEspecialidadRepository.findOne).toHaveBeenCalledWith({
        where: { nombre: 'Cirugia' },
      });
  ...
  describe('findAll', () => {
    it('debería listar todas las especialidades ordenadas', async () => {
      ...
      expect(mockEspecialidadRepository.find).toHaveBeenCalledWith({
        order: { nombre: 'ASC' },
      });
    });
  });
  ...
  describe('update', () => {
    it('debería arrojar error si la especialidad a actualizar no existe', async () => {
      mockEspecialidadRepository.findOne.mockResolvedValue(null);
      await expect(service.update('Falsa', {} as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('debería actualizar la especialidad correctamente', async () => {
      ...
      expect(mockEspecialidadRepository.update).toHaveBeenCalledWith('1', {
        nombre: 'Nueva',
      });
  ...
  describe('remove', () => {
    it('debería arrojar error si la especialidad a eliminar no existe', async () => {
      mockEspecialidadRepository.findOne.mockResolvedValue(null);
      await expect(service.remove('Falsa')).rejects.toThrow(
        BadRequestException,
      );
    });
